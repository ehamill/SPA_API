...
namespace MAWcore6.Controllers
{
    ...
    public class CityController : ControllerBase
    {
        ...
        public async Task<JsonResult> Get()
        {
            ...
            UserCity.Heros = await UpdateHeros(UserCity); //List<Hero> Heros = await GetHeros(UserCity);
            //await UpdateResources(UserCity);
            UserCity.BuildingCosts = GetNewBuildingsCost(UserCity, userResearch); //List<BuildingCost> ListOfBuildingsCost = GetNewBuildingsCost(UserCity, userResearch);
            //Combine Troop and wall defense costs.
            UserCity.Troops = GetTroops(UserCity, userResearch);
            UserCity.WallDefenses = GetWallDefenseTroops(UserCity, userResearch);
        ...
        public async Task<JsonResult> HireHero([FromBody] HireHeroModel model)
        {
           ...
           // UserCity = await db.Cities.Include(c => c.Buildings).Include(c => c.Heros).Where(c => c.UserId == UserId).AsSplitQuery().FirstOrDefaultAsync() ?? await CreateCity(UserId);
            ...
            {
                System.Diagnostics.Debug.WriteLine("Error at CityController HireHero, []: " + ex.Message + ex.InnerException.Message);
                Console.WriteLine(ex.Message);
            }
     ...
     private List<Troop> GetTroops(City city,UserResearch research) {
        ...
        private List<Troop> GetWallDefenseTroops(City city, UserResearch research)
        {
            List<Troop> Troops = new List<Troop>();
        ...
        private async Task<List<Hero>> UpdateHeros(City userCity)
        {
